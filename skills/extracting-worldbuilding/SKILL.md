...
本技能用于“模板目录 + 小说原文”的世界观研究生成。

当前主流程是 **V2 LangExtract**：Python 读取真实模板与长篇原文，调用 LangExtract + MiniMax 逐块、多趟、并行抽取结构化结果，过滤无法回到原文的 `char_interval=None` 条目，合并去重后再由 MiniMax 基于模板推荐结构成文。优先入口是 `scripts/run_extraction.ps1`。

旧 candidate/evidence 正则链与 `batch_plan/subagent` 并行写作链已经被 V2 LangExtract 取代，仅保留为历史/辅助粗筛路径；不要再把它们当作零遗漏抽取主流程。

历史流程是模板驱动的并行编排：Python 生成 `batch-plan.json`，调用方根据 plan 派发多个 subagent，最终每个模板产出一篇 Markdown 文档。
...
在 Windows 上优先使用 LangExtract 入口。开发验证可先用 `-DryRun` 或显式 `-LimitChars`，任何截断都会写入 `run-summary.json`，不得把小样本结果宣称为全书零遗漏。

```powershell
& 'C:\Users\Administrator\plugins\cultivation-worldbuilding-research\scripts\run_extraction.ps1' `
  -Template 'E:\AI_Projects\CultivationWorld\docs\世界观参考\模板\丹药分析模板.md' `
  -SourceFile 'E:\AI_Projects\CultivationWorld\docs\世界观参考\凡人修仙传\凡人修仙传.txt' `
  -OutputDir 'E:\AI_Projects\CultivationWorld\docs\世界观参考\凡人修仙传' `
  -Passes 3 `
  -Workers 4 `
  -MaxCharBuffer 1000
```

若要验证多个代表模板，使用 `-TemplateDir` + `-TemplateLimit`，先跑 3-5 个模板观察成本、块数、抽取条数、去重条数和可视化结果，再决定是否铺开全模板目录。
...
V2 LangExtract 流程中，Python 可以管理编码读取、切块参数、LangExtract 调用、并发参数、可视化、合并去重和 run-summary；API key 只从环境变量或 `.env` 读取，禁止硬编码。

旧 candidate/evidence 抽取仅为 entity_table 类模板寻找漏网名词和证据片段，属于历史/辅助粗筛。叙事 / 关系 / 流程类模板，例如事件因果链、人物关系、修炼历程、职业闭环，不再建议走旧正则链作为主流程。
